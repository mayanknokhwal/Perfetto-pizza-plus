...
function reconcileWalletTranches(wallet) {
    ...
    wallet.transactions.forEach(tx => {
        ...
        if (txType === 'debit' || (tx.type === 'hold' && tx.status === 'LOCKED_HOLD') || (txType === 'hold' && (txStatus === 'locked_hold' || txStatus === 'hold' || txStatus === 'completed' || txStatus === 'completed_hold')) || (txType === 'wallet_hold' && (txStatus === 'locked' || txStatus === 'locked_hold' || txStatus === 'pending'))) {
    ...
    // A debit for Order X CANNOT consume credit earned from Order X
    // Consuming strictly in FIFO order: earliest expiring first
    for (const debit of debits) {
        let needed = debit.amount;
        if (needed <= 0) continue;

        const debitOrderId = String(debit.tx.orderId || '').trim().replace(/^#/, '');

        // Filter and sort available credit tranches for this debit
        const eligible = credits
            .filter(c => {
                if (c.tx.remainingAmount <= 0) return false;
                const creditOrderId = String(c.tx.orderId || '').trim().replace(/^#/, '');
                if (debitOrderId && creditOrderId && debitOrderId === creditOrderId) return false;
                return c.createdTime <= debit.time && c.expiresAtMs > debit.time;
            })
            .sort((a, b) => {
    ...
    // Active unfulfilled holds aggregation:
    const activeHolds = [];
    const seenHoldOrderIds = new Set();
    if (Array.isArray(wallet.transactions)) {
        wallet.transactions.forEach(tx => {
            ...
            const isHoldType = (tType === 'WALLET_HOLD' || tType === 'HOLD' || tType === 'DEBIT');
    ...
    try {
        ...
        if (Array.isArray(storedOrders)) {
            ...
            storedOrders.forEach(o => {
                ...
                const oHoldStat = String(o.walletHoldStatus || o.holdStatus || '').toUpperCase().trim();
                const isRefunded = Boolean(o.walletRefundProcessed || o.walletRefunded);
                const oid = String(o.id || o.orderId || '').replace(/^#/, '').trim();
                if (!oid || seenHoldOrderIds.has(oid)) return;
                const isPendingOrLocked = (oStatus === 'pending' || oHoldStat === 'LOCKED' || oHoldStat === 'LOCKED_HOLD' || !terminalStatuses.includes(oStatus));
                if (isPendingOrLocked && !terminalStatuses.includes(oStatus) && !isRefunded) {
                    const held = Number(o.walletDiscount || o.usedWalletCash || o.usedWallet || 0);
                    if (held > 0) {
                        seenHoldOrderIds.add(oid);
                        activeHolds.push({ orderId: oid, amount: held, status: 'LOCKED' });
                    }
                }
            });
        }
    ...
    const totalActiveHolds = activeHolds.reduce((sum, h) => sum + Number(h.amount || 0), 0);

    let baseCreditPool = 0;
    if (credits.length > 0) {
        baseCreditPool = Math.max(0, activeBalance);
    } else {
...
function getCashbackRewardBoundaries(subtotal, walletConfig = customerWalletConfig) {
    ...
    for (let i = 0; i < sorted.length; i++) {
        const slabMin = sorted[i].minOrder;
        if (subtotal >= slabMin) {
            qualifiedIndex = i;
        } else if (!nextSlab) {
            nextSlab = sorted[i];
...
function getActiveLockedWalletInfo() {
                ...
                if (!terminalStatuses.includes(st) && !isRefunded) {
                    ...
                    if (held > 0) {
                        ...
                        const isReleasedInWallet = currentCustomerWallet && Array.isArray(currentCustomerWallet.transactions) &&
                            currentCustomerWallet.transactions.some(tx => {
                                if (!tx) return false;
                                const txOrd = String(tx.orderId || '').replace(/^#/, '').trim();
                                const txType = String(tx.type || '').toLowerCase().trim();
                                const txStatus = String(tx.status || '').toLowerCase().trim();
                                return (allKeys.has(txOrd) || txOrd === idClean) && (txStatus === 'released' || txStatus === 'cancelled' || txStatus === 'completed' || txType === 'refund');
                            });
    ...
    // Also inspect wallet.transactions for any active locked holds not already counted
    try {
        if (currentCustomerWallet && Array.isArray(currentCustomerWallet.transactions)) {
            currentCustomerWallet.transactions.forEach(tx => {
                if (!tx) return;
                const txType = String(tx.type || '').toUpperCase().trim();
                const txStatus = String(tx.status || '').toUpperCase().trim();
                const holdStatus = String(tx.holdStatus || '').toUpperCase().trim();
                const isHold = (txType === 'WALLET_HOLD' || txType === 'HOLD' || txType === 'DEBIT');
                const isLocked = (txStatus === 'LOCKED' || txStatus === 'LOCKED_HOLD' || txStatus === 'PENDING' || holdStatus === 'LOCKED' || holdStatus === 'LOCKED_HOLD');
                const isTerminal = (txStatus === 'RELEASED' || txStatus === 'CANCELLED' || txStatus === 'COMPLETED' || txStatus === 'DEBITED');
...
function updateCheckoutWalletUI() {
    ...
    // Display inline notice if wallet balance is currently locked in an active order
    let lockedNoticeEl = document.getElementById('checkout-wallet-locked-notice');
    if (hasActiveLock) {
        if (!lockedNoticeEl) {
            lockedNoticeEl = document.createElement('div');
            lockedNoticeEl.id = 'checkout-wallet-locked-notice';
            lockedNoticeEl.className = 'wallet-locked-inline-notice';
            walletCard.appendChild(lockedNoticeEl);
        }
    ...
    // Hard-Lock Checkout Checkbox when Balance is Held or Depleted:
    // If usableBalance <= 0 OR there is an active order with funds currently locked:
    // - Disable the wallet redemption toggle/checkbox: checkbox.disabled = true;
    // - Uncheck the toggle: checkbox.checked = false;
    // - Remove any wallet discount from the bill calculation: walletDiscount = 0;
    // - Prevent the user from manually toggling or bypassing the lock.
    if (availableBalance <= 0 || maxRedeemable <= 0 || hasActiveLock) {
        isWalletRedemptionSelected = false;
        appliedWalletDiscountAmount = 0;
        if (checkbox) {
        ...
        if (checkLabelWrap) {
            ...
            checkLabelWrap.style.opacity = '0.45';
    ...
    if (isWalletRedemptionSelected && maxRedeemable > 0 && !hasActiveLock) {
...
async function createWalletHoldRecord(phone, amount, orderId) {
    const cleanPhone = String(phone || '').replace(/[^0-9]/g, '').slice(-10);
    const debitAmt = Number(amount) || 0;
    if (debitAmt <= 0) return;

    const effectiveOrderId = String(orderId || '').replace(/^#/, '');
    ...
    const existingTx = currentCustomerWallet.transactions;

    // Idempotency check: prevent duplicate hold for the exact same order
    if (effectiveOrderId && effectiveOrderId !== 'ORDER' && effectiveOrderId !== '--') {
        const alreadyDebited = existingTx.some(tx => tx && 
            (tx.type === 'debit' || tx.type === 'hold' || tx.type === 'WALLET_HOLD') && 
            String(tx.orderId || '').replace(/^#/, '') === effectiveOrderId && 
            tx.status !== 'released' && tx.status !== 'cancelled'
        );
        if (alreadyDebited) {
            console.log(`[WALLET] Order #${effectiveOrderId} has already been held/debited. Skipping duplicate.`);
            return;
        }
    }
    ...
    // Reconcile tranches with FIFO deduction against credit tranches (reducing spendable balance)
    const updatedWalletBalance = reconcileWalletTranches(currentCustomerWallet);

    try {
        localStorage.setItem('perfetto_wallet_balance', String(updatedWalletBalance));
    } catch (e) {}
    ...
    try {
        ...
        if (fs && cleanPhone) {
            const walletRef = fs.collection('wallets').doc(cleanPhone);
            await walletRef.set({
                phone: cleanPhone,
        ...
        console.warn('Error recording wallet hold in Firestore:', err);
    }
}
window.createWalletHoldRecord = createWalletHoldRecord;

async function debitCustomerWallet(phone, amount, orderId) {
    return createWalletHoldRecord(phone, amount, orderId);
}
window.debitCustomerWallet = debitCustomerWallet;

function commitWalletHold(orderId) {
    if (!currentCustomerWallet || !Array.isArray(currentCustomerWallet.transactions)) return;
    const effectiveOrderId = String(orderId || '').replace(/^#/, '');
    const holdTx = currentCustomerWallet.transactions.find(tx => tx && 
        (tx.type === 'hold' || tx.type === 'WALLET_HOLD' || tx.type === 'debit' || String(tx.id || '') === `tx_hold_${effectiveOrderId}`) && 
...
function releaseWalletHold(orderId, refundAmount, customExpiresAt) {
    if (!currentCustomerWallet) currentCustomerWallet = { balance: 0, nonExpiredBalance: 0, transactions: [] };
    if (!Array.isArray(currentCustomerWallet.transactions)) currentCustomerWallet.transactions = [];
    ...
    const recoveredExpiresAt = (typeof calculateRecoveredExpiry === 'function')
        ? calculateRecoveredExpiry(origExpiry, nowMs)
    ...
    const origExpMs = origExpiry ? parseTs(origExpiry) : NaN;
    const isOrigExpired = isNaN(origExpMs) || origExpMs <= nowMs;
    const hasGraceApplied = isOrigExpired || (origExpMs - nowMs) < (24 * 60 * 60 * 1000);

    if (holdTx) {
        holdTx.status = 'released';
        holdTx.recoveredExpiresAt = recoveredExpiresAt;
        holdTx.graceApplied = hasGraceApplied;
    }
    ...
    if (!alreadyRefunded && refundAmt > 0) {
        const refundTx = {
            ...
            amount: refundAmt,
            orderId: effectiveOrderId,
            title: `+₹${refundAmt} Refund`,
            description: `+₹${refundAmt} Refund for Order #${effectiveOrderId}`,
            status: 'completed',
    ...
    const currExpMs = currentCustomerWallet.expiresAt ? parseTs(currentCustomerWallet.expiresAt) : NaN;
    if (isNaN(currExpMs) || currExpMs <= nowMs || (hasGraceApplied && new Date(recoveredExpiresAt).getTime() > currExpMs)) {
        currentCustomerWallet.expiresAt = recoveredExpiresAt;
...
/**
 * Returns the exact reward and cashback slab value configured in the remote Firestore database
 * matching the qualified cart threshold, eliminating hardcoded fallbacks and random ranges.
 * @param {number} subtotal
 * @param {Object} [walletConfig]
 * @returns {number}
 */
function calculateOrderCashback(subtotal, walletConfig = customerWalletConfig) {
    ...
    const exactReward = Number(boundaries.slab.cashback !== undefined ? boundaries.slab.cashback : boundaries.max) || 0;
    return Math.max(0, Math.round(exactReward));
}
    ...
    container.innerHTML = sortedTxList.map(tx => {
        ...
        if (isRefund) {
            ...
            if (tx.status === 'released') {
                expiryNotice = '<span class="tx-badge-used"><i class="fa-solid fa-rotate-left"></i> Hold Released</span>';
            } else {
        ...
        if (!txTitle) {
            ...
            else txTitle = `Used for Order #${tx.orderId || ''}`;
        }

        const amountPrefix = isPositive ? '+' : (isHold ? '' : '-');
        ...
        return `
            <div class="wallet-tx-item ${rowClass}">
                <div class="wallet-tx-left">
                    <span class="wallet-tx-title">${escapeHtml(txTitle)}</span>
...
try {
    window.addEventListener('storage', (e) => {
        if (e.key === 'perfetto_max_offers_per_order' && e.newValue) {
...
function setCustomerMaxOffersPerOrder(val) {
...
window.setCustomerMaxOffersPerOrder = setCustomerMaxOffersPerOrder;
...
function updateSpendHungerBar() {
    ...
    const isDismissed = (function () {
        try { return sessionStorage.getItem('banner2SpendBarDismissed') === 'true'; } catch (e) { return false; }
    })();
    ...
    const currentGiftItems = (Array.isArray(cart) ? cart : []).filter(item => item.isFreeGift);
    const totalClaimedGifts = currentGiftItems.reduce((sum, item) => sum + (Number(item.qty) || 1), 0);

    // Inactive by default: If customer never tapped Slot 2 and has no earned free gifts, hide hunger bar completely
    if (!isBanner2OfferActive && totalClaimedGifts === 0) {
        hungerWrapper.style.display = 'none';
    ...
    if (maxUnlockedTiers === 0) {
        // Below Tier 1
        const remaining = baseMinSpend - qualifyingPaidTotal;
        const progressPct = Math.min(100, Math.max(0, Math.round((qualifyingPaidTotal / baseMinSpend) * 100)));
        ...
        if (hungerDeficit) hungerDeficit.textContent = `Add ₹${remaining} more to unlock your FREE ${rewardName}!`;
        if (hungerRight) {
            hungerRight.innerHTML = `
                <span class="spend-hunger-pill">${progressPct}%</span>
                <button type="button" class="btn-hunger-dismiss" onclick="dismissSpendHungerBar(event)" aria-label="Dismiss Offer" title="Dismiss Offer">
                    <i class="fa-solid fa-xmark"></i>
                </button>
            `;
        }
        window.__lastAutoPoppedTier = 0;
    } else if (maxUnlockedTiers < maxQty) {
        // Tier 1 to N met, but still below maxQty cap
        const currentTier = maxUnlockedTiers;
        const nextTier = currentTier + 1;
        const nextTargetSpend = nextTier * baseMinSpend;
        const remainingForNext = nextTargetSpend - qualifyingPaidTotal;

        // Meter shows progress towards next tier
        const tierSpend = qualifyingPaidTotal - (currentTier * baseMinSpend);
        const progressPct = Math.min(100, Math.max(0, Math.round((tierSpend / baseMinSpend) * 100)));
        ...
        if (hungerIcon) hungerIcon.textContent = '🎁';
        if (hungerTitle) hungerTitle.textContent = `🎉 TIER ${currentTier} UNLOCKED!`;
        if (hungerDeficit) hungerDeficit.textContent = `Tier ${currentTier} Unlocked! Add ₹${remainingForNext} more to unlock your ${getOrdinal(nextTier)} FREE ${rewardName}!`;

        if (totalClaimedGifts < currentTier) {
            // Unclaimed gift(s) waiting
            const nextGiftToClaim = totalClaimedGifts + 1;
            if (hungerRight) {
                hungerRight.innerHTML = `
                    <button type="button" class="btn-hunger-claim" onclick="openFreeGiftSelectionModal()">
                        <i class="fa-solid fa-gift"></i> Claim ${getOrdinal(nextGiftToClaim)}
                    </button>
                    <button type="button" class="btn-hunger-dismiss" onclick="dismissSpendHungerBar(event)" aria-label="Dismiss Offer" title="Dismiss Offer">
                        <i class="fa-solid fa-xmark"></i>
                    </button>
            ...
            // All unlocked gifts claimed
            if (hungerRight) {
                hungerRight.innerHTML = `
                    <span class="spend-hunger-pill pill-unlocked">${progressPct}%</span>
                    <button type="button" class="btn-hunger-change" onclick="openFreeGiftSelectionModal()">
                        <i class="fa-solid fa-gift"></i> Gifts (${totalClaimedGifts})
                    </button>
                    <button type="button" class="btn-hunger-dismiss" onclick="dismissSpendHungerBar(event)" aria-label="Dismiss Offer" title="Dismiss Offer">
                        <i class="fa-solid fa-xmark"></i>
                    </button>
        ...
        // Auto-pop modal for this new tier if not yet claimed
        if (totalClaimedGifts < currentTier) {
            const lastPoppedTier = window.__lastAutoPoppedTier || 0;
            if (currentTier > lastPoppedTier && !isAutoOpeningFreeGiftModal) {
                if (canClaimBannerOffer('freeGift')) {
                    window.__lastAutoPoppedTier = currentTier;
        ...
        // Max Tier reached (maxUnlockedTiers >= maxQty)
        if (hungerFill) {
            hungerFill.style.width = '100%';
            hungerFill.classList.add('fill-complete');
        }
        if (hungerIcon) hungerIcon.textContent = '👑';
        if (hungerTitle) hungerTitle.textContent = `🎉 ALL ${maxQty} TIERS UNLOCKED!`;

        if (totalClaimedGifts < maxQty) {
            const nextGiftToClaim = totalClaimedGifts + 1;
            if (hungerDeficit) hungerDeficit.textContent = `Claim your remaining ${maxQty - totalClaimedGifts} FREE ${rewardName}(s)!`;
            if (hungerRight) {
                hungerRight.innerHTML = `
                    <button type="button" class="btn-hunger-claim" onclick="openFreeGiftSelectionModal()">
                        <i class="fa-solid fa-gift"></i> Claim ${getOrdinal(nextGiftToClaim)}
            ...
            const lastPoppedTier = window.__lastAutoPoppedTier || 0;
            if (maxQty > lastPoppedTier && !isAutoOpeningFreeGiftModal) {
                if (canClaimBannerOffer('freeGift')) {
                    window.__lastAutoPoppedTier = maxQty;
                    isAutoOpeningFreeGiftModal = true;
                    setTimeout(() => {
                        isAutoOpeningFreeGiftModal = false;
                        const freeModal = document.getElementById('free-gift-modal');
            ...
            if (hungerDeficit) hungerDeficit.textContent = `Max offer limit reached! ${totalClaimedGifts} FREE ${rewardName}${totalClaimedGifts > 1 ? 's' : ''} in cart!`;
            if (hungerRight) {
                hungerRight.innerHTML = `
                    <button type="button" class="btn-hunger-change" onclick="openFreeGiftSelectionModal()">
                        <i class="fa-solid fa-gift"></i> Free Gifts (${totalClaimedGifts})
                    </button>
                    <button type="button" class="btn-hunger-dismiss" onclick="dismissSpendHungerBar(event)" aria-label="Dismiss Offer" title="Dismiss Offer">
                        <i class="fa-solid fa-xmark"></i>
                    </button>
...
function removeFreeGiftFromCart(targetIndex) {
    if (typeof targetIndex === 'number' && targetIndex >= 0 && cart[targetIndex] && cart[targetIndex].isFreeGift) {
        if ((cart[targetIndex].qty || 1) > 1) {
    ...
    showToast('Free gift removed from cart');
}
window.removeFreeGiftFromCart = removeFreeGiftFromCart;

function removeBogoComboFromCart() {
...
window.removeBogoComboFromCart = removeBogoComboFromCart;

function dismissSpendHungerBar(event) {
    if (event) {
        event.preventDefault();
        event.stopPropagation();
    }
...
window.dismissSpendHungerBar = dismissSpendHungerBar;
window.cancelSpendOffer = dismissSpendHungerBar;
...
function updateCartUI() {
    ...
    // Calculate qualifyingPaidTotal (sum of standard paid items in cart, strictly excluding free gifts and BOGO rewards)
    ...
    if (bogoRewardItems.length > 0) {
        ...
        bogoRewardItems.forEach(rewardItem => {
            const linkedComboId = rewardItem.bogoComboId;
            const qualifyingItemsInCombo = cart.filter(item =>
    ...
    if (freeGiftContainer) {
        ...
        if ((isBanner2OfferActive || totalClaimedGifts > 0) && cart.length > 0) {
            const isPizza = isCategoryMatch(slot2Config.rewardCategory, 'Pizza');
            const rewardName = isPizza
                ? `${(slot2Config.rewardPizzaSize || 'Medium').charAt(0).toUpperCase() + (slot2Config.rewardPizzaSize || 'Medium').slice(1)} Pizza`
            ...
            if (maxAllowedGifts > totalClaimedGifts) {
                ...
                freeGiftContainer.style.display = 'block';
                freeGiftContainer.innerHTML = `
                    <div class="cart-free-gift-unlock-banner in-progress">
                        <div class="gift-banner-left">
                            <span class="gift-banner-icon">🎯</span>
                            <div class="gift-banner-text">
        ...
        cartContainer.innerHTML = cart.map((item, index) => {
            ...
            if (isAnyFree) {
                if (item.price > 0) {
                    ...
                    priceMarkup = `<span class="cart-item-price" style="color: #10b981; font-weight:800;">FREE</span>`;
                }
            } else {
                const hasDiscount = Boolean(item.originalPrice && item.originalPrice > item.price);
            ...
            if (isAnyFree) {
                ...
                <div class="cart-item-card cart-item-free-gift">
                    ...
                    <div class="free-gift-cart-controls">
                        ...
                        <button type="button" class="btn-cart-remove-deal" onclick="${removeAction}" title="${isBogo ? 'Remove BOGO Combo' : 'Remove Free Gift'}">
...
window.placeOrder = handleSelectCodPayment;
...
function executeOrderPlacement(profile, paymentMethod = 'Cash on Delivery', paymentStatus = 'Cash on Delivery', specificOrderId = null, clearCartNow = true) {
    ...
    const { lockedAmount: activeLockedWalletAmount } = (typeof getActiveLockedWalletInfo === 'function') ? getActiveLockedWalletInfo() : { lockedAmount: 0 };
    const hasActiveOrderLock = (activeLockedWalletAmount > 0);

    const walletDiscountToApply = (isWalletRedemptionSelected && !hasActiveOrderLock && currentAvailableFunds >= appliedWalletDiscountAmount && currentAvailableFunds > 0)
    ...
    const newOrder = {
        ...
        holdStatus: (walletDiscountToApply > 0 ? 'LOCKED' : null),
        walletHoldExpiresAt: (walletDiscountToApply > 0 && currentCustomerWallet) ? (currentCustomerWallet.expiresAt || null) : null,
        walletOriginalExpiresAt: (walletDiscountToApply > 0 && currentCustomerWallet) ? (currentCustomerWallet.expiresAt || null) : null,
...
function openScratchCardForOrder(orderId) {
    ...
    if (targetOrder) {
        ...
        const fallbackSubtotal = 500;
        const exactReward = calculateOrderCashback(fallbackSubtotal);
        openScratchCardModal({
            id: orderId,
            orderId: orderId,
            subtotal: fallbackSubtotal,
            earnedCashback: exactReward,
            wonCashback: exactReward,
            status: 'completed'
        });
    }
}
...
// --------------------------------------------------------------------------
// CUSTOMER-SIDE 100-MINUTE AUTO-EXPIRATION & 24-HOUR GRACE RECOVERY
// --------------------------------------------------------------------------
...
function getOrderCountdownPillHTML(order, nowMs = Date.now()) {
    ...
    return `<span class="order-countdown-pill ${pillClass}" title="100-Minute Kitchen Window">⏱ ${countdownText}</span>`;
}
...
async function autoRejectExpiredCustomerOrder(order) {
    ...
    if (refundAmount > 0 && !isAlreadyRefunded) {
        ...
        if (typeof releaseWalletHold === 'function') {
            releaseWalletHold(orderId, refundAmount, originalExpiresAt);
        } else if (currentCustomerWallet) {
            const currentBal = Number(currentCustomerWallet.balance || 0);
            currentCustomerWallet.balance = currentBal + refundAmount;
            currentCustomerWallet.expiresAt = recoveredExpiresAt;
            currentCustomerWallet.expired = false;
            if (!Array.isArray(currentCustomerWallet.transactions)) currentCustomerWallet.transactions = [];
    ...
    try {
        ...
        const idx = storedOrders.findIndex(o => String(o.id || o.orderId) === orderId);
        if (idx >= 0) {
            storedOrders[idx] = { ...storedOrders[idx], ...order };
    ...
    // Remote Firestore batch update
    if (customerFirestore) {
        try {
            const batch = customerFirestore.batch();
            const orderRef = customerFirestore.collection('orders').doc(orderId);
            ...
            if (order.scratchCard) {
                updatePayload['scratchCard.status'] = 'CANCELLED';
                updatePayload['scratchCard.voided'] = true;
                updatePayload['scratchCard.wonAmount'] = 0;
            ...
            if (refundAmount > 0 && customerPhone && !isAlreadyRefunded) {
                ...
                const userPrefixedRef = customerFirestore.collection('users').doc(`phone_${customerPhone}`);
                batch.set(userPrefixedRef, { balance: incrementFn, walletBalance: incrementFn, expiresAt: recoveredExpiresAt, expired: false, updatedAt: serverTs }, { merge: true });
                ...
                const userRawRef = customerFirestore.collection('users').doc(customerPhone);
                batch.set(userRawRef, { balance: incrementFn, walletBalance: incrementFn, expiresAt: recoveredExpiresAt, expired: false, updatedAt: serverTs }, { merge: true });
                ...
                const walletRef = customerFirestore.collection('wallets').doc(customerPhone);
                batch.set(walletRef, { phone: customerPhone, balance: incrementFn, expiresAt: recoveredExpiresAt, updatedAt: serverTs }, { merge: true });
            ...
            batch.set(orderRef, updatePayload, { merge: true });
            await batch.commit();
            console.log(`✅ [CUSTOMER EVAL] Atomic rejection and refund persisted to Firestore for #${orderId}`);
        } catch (fsErr) {
            console.warn(`[CUSTOMER EVAL] Firestore batch write note for #${orderId}:`, fsErr);
        }
    }
...
// --------------------------------------------------------------------------
// BANNER SLOT 2 SPEND TARGET & FREE GIFT REWARD ENGINE
// --------------------------------------------------------------------------
let currentFreeGiftState = {
    selectedItem: null,
    selectedAddons: { cheese: false, spicy: false, mayo: false, iceCream: false },
    eligibleItems: [],
    slotConfig: null
...
window.getBannerSlot2Config = getBannerSlot2Config;
...
function openFreeGiftSelectionModal(editIndex) {
    ...
    const config = getBannerSlot2Config();
    currentFreeGiftState.slotConfig = config;
    ...
    if (isEditing && cart[editIndex]) {
        ...
        currentFreeGiftState.selectedItem = eligible[0] || null;
    }

    renderFreeGiftItemsGrid();
...
window.openFreeGiftSelectionModal = openFreeGiftSelectionModal;
...
function renderFreeGiftItemsGrid() {
    const grid = document.getElementById('free-gift-items-grid');
    if (!grid) return;

    const { eligibleItems, selectedItem, slotConfig } = currentFreeGiftState;
    ...
    const pizzaSize = (slotConfig && slotConfig.rewardPizzaSize) ? slotConfig.rewardPizzaSize.toLowerCase() : 'medium';
    ...
    grid.innerHTML = eligibleItems.map(item => {
        ...
        if (slotConfig && slotConfig.rewardType === 'pizza') {
...
function onSelectFreeGiftItem(itemIdOrName) {
    const found = currentFreeGiftState.eligibleItems.find(i => (i.id === itemIdOrName || i.name === itemIdOrName));
    if (!found) return;
    currentFreeGiftState.selectedItem = found;
    currentFreeGiftState.selectedAddons = { cheese: false, spicy: false, mayo: false, iceCream: false };
    renderFreeGiftItemsGrid();
    renderFreeGiftAddonsAndPricing();
}
window.onSelectFreeGiftItem = onSelectFreeGiftItem;

function renderFreeGiftAddonsAndPricing() {
    const { selectedItem, slotConfig, selectedAddons } = currentFreeGiftState;
    const addonsBox = document.getElementById('free-gift-addons-box');
    const addonsChipsContainer = document.getElementById('free-gift-addon-chips');
    const strikePriceEl = document.getElementById('free-gift-strike-price');
    const finalPriceEl = document.getElementById('free-gift-final-price');
    const breakdownTagEl = document.getElementById('free-gift-breakdown-tag');
    ...
    // Calculate base original price
    let origBasePrice = Number(selectedItem.price) || 0;
    const isPizza = (slotConfig && slotConfig.rewardType === 'pizza') || (selectedItem.category || '').toLowerCase() === 'pizza';
    let pizzaSizeKey = 'M';
    if (isPizza) {
        const pSize = (slotConfig && slotConfig.rewardPizzaSize) ? slotConfig.rewardPizzaSize.toLowerCase() : 'medium';
    ...
    // Determine available add-ons for the item's category
    ...
    // Calculate total add-ons cost (100% full rate - Add-on Protection)
...
function confirmClaimFreeGift() {
    ...
    // Build add-ons list & calculate add-on price
    const catName = selectedItem.category || (isPizza ? 'Pizza' : 'Shake');
    let addonConfig = (typeof getCustomerCategoryAddons === 'function') ? getCustomerCategoryAddons(catName) : {};
    if (isPizza && addonConfig.sizes && addonConfig.sizes[pizzaSizeKey]) {
        addonConfig = addonConfig.sizes[pizzaSizeKey];
...
function syncCustomerPhoneOrders(remoteOrders, verifiedPhone) {
    try {
        ...
        remoteOrders.forEach(o => {
            ...
            if (clearedSet.has(id) && terminalStatuses.has(st)) return;
            map.set(id, o);
            [o.id, o.orderId, o.firestoreDocId, o.docId].filter(Boolean).forEach(k => {
                const s = String(k).trim();
                aliasMap.set(s, id);
                aliasMap.set(s.replace(/^#/, ''), id);
            });
        });
        ...
        storedOrders.forEach(o => {
            ...
            const candKeys = [o.id, o.orderId, o.firestoreDocId, o.docId].filter(Boolean).map(k => String(k).trim());
            let matchedKey = null;
            for (const k of candKeys) {
                if (aliasMap.has(k)) { matchedKey = aliasMap.get(k); break; }
                const cl = k.replace(/^#/, '');
                if (aliasMap.has(cl)) { matchedKey = aliasMap.get(cl); break; }
            }
...
function handleRealtimeCustomerOrderUpdate(orderId, freshOrderData) {
    try {
        ...
        if (target) {
            ...
            if (freshOrderData.rejectionReason) {

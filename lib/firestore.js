...
// Global TTL Cache Stores (60s TTL to prevent runaway Firestore REST read bills)
const FIRESTORE_READ_CACHE_TTL_MS = 60000;
const FIRESTORE_NEGATIVE_CACHE_TTL_MS = 30000;
...
/**
 * Fetches a single document from Firestore or in-memory fallback (with 60s TTL and negative caching)
 */
async function getFirestoreDoc(collection, docId, forceFresh = false) {
    ...
    // Return cached document or negative lookup if within TTL
    if (!forceFresh && global.__firestoreMemoryCache.has(cacheKey)) {
        const cachedVal = global.__firestoreMemoryCache.get(cacheKey);
        const ttl = (cachedVal === null) ? FIRESTORE_NEGATIVE_CACHE_TTL_MS : FIRESTORE_READ_CACHE_TTL_MS;
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Negative caching: Cache 404/not found for 30s to prevent rapid re-fetching loops
            global.__firestoreMemoryCache.set(cacheKey, null);
            global.__firestoreDocCacheTimestamps.set(cacheKey, Date.now());
            return null;
        }
    } catch (err) {
...
async function setFirestoreDoc(collection, docId, data, merge = true) {
    ...
    let url = `${FIRESTORE_BASE_URL}/${collection}/${encodeURIComponent(docId)}?key=${FIREBASE_CONFIG.apiKey}`;
    let docPayload;
    ...
    if (merge && data && typeof data === 'object' && !Array.isArray(data)) {
        ...
        if (keys.length > 0) {
            ...
            url += `&${maskParams}`;
            docPayload = jsObjectToFirestoreDoc(data);
        } else {
            docPayload = jsObjectToFirestoreDoc(mergedData);
        }
    } else {
        docPayload = jsObjectToFirestoreDoc(mergedData);
    }

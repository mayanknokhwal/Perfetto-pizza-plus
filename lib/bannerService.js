...
const DEFAULT_DAILY_BANNERS = [
    ...
    { id: 'b3', url: 'https://i.ibb.co/VYqnBKbM/free-medium-pizza.png', enabled: true, offerMode: 'pizza_stepdown', targetCategory: 'pizza' },
 ...
 * - Slot 3 preserves offerMode, targetCategory
 * 
 * @param {Array} rawBanners - Raw input banners array
 * @returns {Array<{id: string, url: string, enabled: boolean, targetProductId?: string, discountPercent?: number, minSpend?: number, rewardType?: string, rewardCategory?: string, rewardPizzaSize?: string, offerMode?: string, targetCategory?: string}>} Validated 4 banners array
 */
    ...
    try {
        ...
        if (doc && Array.isArray(doc.banners) && doc.banners.length > 0) {
            ...
            if (doc.slot3 && normalized[2]) {
                if (doc.slot3.offerMode) {
                    normalized[2].offerMode = doc.slot3.offerMode;
                }
                if (doc.slot3.targetCategory) {
                    normalized[2].targetCategory = doc.slot3.targetCategory;
                }
            }
...
async function saveDailyBannersToFirestore(bannersList) {
    ...
    const isSlot3BogoCat = (validated[2].offerMode === 'category_bogo');
    const slot3Data = {
        imageUrl: validated[2].url,
        url: validated[2].url,
        offerMode: validated[2].offerMode || 'pizza_stepdown',
        targetCategory: isSlot3BogoCat ? (validated[2].targetCategory || 'Shake') : 'pizza',

...
const validateAndNormalizeBanners = sanitizeBannerList;
...
async function fetchDailyBannersFromFirestore() {
    try {
        ...
        if (doc && Array.isArray(doc.banners) && doc.banners.length > 0) {
            ...
            if (doc && (doc.max_offers_per_order !== undefined || doc.maxOffersPerOrder !== undefined || doc.maxClaimableOffers !== undefined)) {
                global.__perfettoMaxOffersPerOrder = Math.max(1, Math.min(3, parseInt(doc.max_offers_per_order || doc.maxOffersPerOrder || doc.maxClaimableOffers, 10) || 2));
            } else {
                global.__perfettoMaxOffersPerOrder = 2;
            }
            // Enforce business rule: fixed quantity of 1 per claimed offer
...
async function saveDailyBannersToFirestore(bannersList, extraPayload = {}) {
    ...
    const slot3In = extraPayload.slot3 || {};
    const buyCat = slot3In.buyCategory || extraPayload.buyCategory || validated[2].buyCategory || 'Momos';
    const buyQty = Number(slot3In.buyQty || extraPayload.buyQty || validated[2].buyQty) || 2;
    const rewCat = slot3In.freeCategory || slot3In.rewardCategory || extraPayload.freeCategory || extraPayload.rewardCategory || validated[2].rewardCategory || 'Shake';
    const freeQty = Number(slot3In.freeQty || extraPayload.freeQty || validated[2].freeQty) || 1;

    validated[2].buyCategory = buyCat.toLowerCase() === 'pizza' ? 'Momos' : buyCat;
    validated[2].buyQty = buyQty;
    validated[2].rewardCategory = rewCat.toLowerCase() === 'pizza' ? 'Shake' : rewCat;
    validated[2].freeQty = freeQty;

    const slot3Data = {
        imageUrl: validated[2].url,
        url: validated[2].url,
        buyCategory: validated[2].buyCategory,
        buyQty: validated[2].buyQty,
        rewardCategory: validated[2].rewardCategory,
        freeQty: validated[2].freeQty,
        active: validated[2].enabled !== false,
        enabled: validated[2].enabled !== false
    };

    const maxOffers = Math.max(1, Math.min(3, parseInt(extraPayload.max_offers_per_order || extraPayload.maxOffersPerOrder || extraPayload.maxClaimableOffers || global.__perfettoMaxOffersPerOrder || 1, 10) || 1));

    ...
    try {
        ...
        if (doc && Array.isArray(doc.banners) && doc.banners.length > 0) {
            ...
            if (doc && (doc.max_qty_per_offer !== undefined || doc.maxQtyPerOffer !== undefined)) {
                global.__perfettoMaxQtyPerOffer = Math.max(1, Math.min(9, parseInt(doc.max_qty_per_offer || doc.maxQtyPerOffer, 10) || 4));
            } else {
                global.__perfettoMaxQtyPerOffer = 4;
            }
...
async function saveDailyBannersToFirestore(bannersList, extraPayload = {}) {
    ...
    const maxOffers = Math.max(1, Math.min(3, parseInt(extraPayload.max_offers_per_order || extraPayload.maxOffersPerOrder || extraPayload.maxClaimableOffers || global.__perfettoMaxOffersPerOrder || 2, 10) || 2));
    global.__perfettoMaxOffersPerOrder = maxOffers;

    const maxQty = Math.max(1, Math.min(9, parseInt(extraPayload.max_qty_per_offer || extraPayload.maxQtyPerOffer || global.__perfettoMaxQtyPerOffer || 4, 10) || 4));

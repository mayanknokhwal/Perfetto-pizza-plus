/**
 * Vercel Serverless Function: Admin Authentication & Role Management API
 * Route: /api/admin-auth
 * 
 * Endpoints:
 * - GET  /api/admin-auth?email=<email> : Check role & status ("active", "pending", "rejected")
 * - GET  /api/admin-auth?list=all      : List all team members and pending requests (restricted to Master Admin)
 * - POST /api/admin-auth               : Upsert login attempt with Google profile details (email, fullName, photoURL, firebaseUid)
 * - PATCH /api/admin-auth              : Approve / assign role / reject pending requests (Master Admin only)
 * - DELETE /api/admin-auth             : Remove staff member (Master Admin only)
 */
...
module.exports = async function handler(req, res) {
    ...
    res.setHeader(
        ...
        'X-CSRF-Token, X-Requested-With, Accept, Accept-Version, Content-Length, Content-MD5, Content-Type, Date, X-Api-Version, Authorization, x-admin-email, x-requester-email, x-user-email'
    );
    ...
    try {
        ...
        if (req.method === 'GET') {
            ...
            // A. LIST ALL TEAM MEMBERS & PENDING REQUESTS (Restricted to Master Admin)
            if (list === 'all' || list === 'team' || list === 'true') {
                ...
                if (!hasMaster) {
                    ...
                    // Ensure Master Admin record in results always has proper role & active status
            ...
            // Unconditional, instant access for Primary Master Admin
        ...
        if (req.method === 'POST') {
            ...
            // Non-master Google users attempting to sign in
            ...
            // Preserve assigned role and status if user was already reviewed/approved
            const assignedRole = existingUser ? existingUser.role : 'Pending';
            const assignedStatus = existingUser ? existingUser.status : 'pending';
        ...
        // ----------------------------------------------------------------------
        // 3. PATCH: Approve (Assign Role) or Reject Pending Requests (Master Admin Only)
        // ----------------------------------------------------------------------
        if (req.method === 'PATCH') {
            let body = req.body;
            if (typeof body === 'string') {
            ...
            // Master Admin cannot be modified or demoted
            ...
            // Handle actions: 'approve', 'reject', or explicit 'role' and 'status'
        ...
        // ----------------------------------------------------------------------
        // 4. DELETE: Remove a staff record (Master Admin Only)
        // ----------------------------------------------------------------------
        ...
        console.error('Error in /api/admin-auth handler:', error);

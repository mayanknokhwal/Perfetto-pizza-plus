/**
 * Vercel Serverless Function: Menu API (GET, POST, PUT, PATCH)
 * Route: /api/menu
 */
...
// In-memory runtime state for offline/fallback resilience across devices
...
module.exports = async function handler(req, res) {
    ...
    try {
        ...
        // -------------------------------------------------------------
        // 1. GET: Fetch Live Menu Items (Auto-seeds if database is empty)
        // -------------------------------------------------------------
        if (req.method === 'GET') {
            const { category } = req.query || {};

            if (!db) {
                let items = global.__perfettoMenuState;
                if (category) items = items.filter(i => i.category === category);

                return res.status(200).json({
                    success: true,
                    isFallback: true,
                    message: 'MongoDB in fallback mode. Serving active server menu state.',
                    count: items.length,
                    items: items,
                });
            }
            ...
            let items = await MenuItem.find(filter).sort({ order: 1, _id: 1 }).lean();

            // Auto-seed database if collection is empty
            if (!items || items.length === 0) {
                console.log('Seeding initial menu items to MongoDB Atlas...');
                const seedDocs = DEFAULT_MENU_ITEMS.map((item, idx) => ({
                    ...item,
                    order: idx,
                }));
                try {
                    await MenuItem.insertMany(seedDocs, { ordered: false });
                    items = await MenuItem.find(filter).sort({ order: 1, _id: 1 }).lean();
                } catch (seedErr) {
                    console.warn('Notice during auto-seeding:', seedErr.message);
                    items = await MenuItem.find(filter).sort({ order: 1, _id: 1 }).lean();
                }
            }

            // Ensure clean frontend compatibility
            const cleanItems = (items && items.length > 0 ? items : DEFAULT_MENU_ITEMS).map(item => ({
                id: item.id,
                name: item.name,
                category: item.category,
                isMultiSize: Boolean(item.isMultiSize),
                price: Number(item.price || 0),
                prices: item.prices || { S: 199, M: 299, L: 399 },
            ...
            global.__perfettoMenuState = cleanItems;
        ...
        // -------------------------------------------------------------
        // 2. PATCH: Instant Single-Item Update (Availability Toggle / Price)
        // -------------------------------------------------------------
        if (req.method === 'PATCH') {
            ...
            let itemIndex = global.__perfettoMenuState.findIndex(i => i.id === targetId);
            if (itemIndex >= 0) {
                if (available !== undefined) global.__perfettoMenuState[itemIndex].available = Boolean(available);
                if (price !== undefined) global.__perfettoMenuState[itemIndex].price = Number(price);
                if (prices !== undefined) global.__perfettoMenuState[itemIndex].prices = prices;
                if (name !== undefined) global.__perfettoMenuState[itemIndex].name = name;
                if (desc !== undefined) global.__perfettoMenuState[itemIndex].desc = desc;
                if (img !== undefined) global.__perfettoMenuState[itemIndex].img = img;
                if (isMultiSize !== undefined) global.__perfettoMenuState[itemIndex].isMultiSize = Boolean(isMultiSize);
            } else {
                global.__perfettoMenuState.push({
                    id: targetId,
                    name: name || 'Food Item',
                    category: 'Pizza',
                ...
                itemIndex = global.__perfettoMenuState.length - 1;
            }

            const updatedItem = global.__perfettoMenuState[itemIndex];

            if (!db) {
                return res.status(200).json({
                    success: true,
                    isFallback: true,
                    message: `Item '${id}' updated in active server state (Fallback mode)`,
                    item: updatedItem,
                });
            }
            ...
            if (isMultiSize !== undefined) updateFields.isMultiSize = Boolean(isMultiSize);
            ...
            return res.status(200).json({
                success: true,
                message: `Menu item '${id}' successfully updated in MongoDB Atlas`,
                item: {
                    id: updated.id,
                    name: updated.name,
                    category: updated.category,
                    isMultiSize: Boolean(updated.isMultiSize),
        ...
        // -------------------------------------------------------------
        // 3. PUT / POST: Bulk Menu Sync & Update (or Reset to Defaults)
        // -------------------------------------------------------------
        if (req.method === 'PUT' || req.method === 'POST') {
            ...
            if (!db) {
                return res.status(200).json({
                    success: true,
                    isFallback: true,
                    message: isReset ? 'Menu reset in active server state' : 'Bulk menu updated in active server state',
                    count: rawItems.length,
                    items: global.__perfettoMenuState,
                });
            }

            // Perform batch upsert in MongoDB Atlas
            const bulkOps = rawItems.map((item, idx) => ({
                updateOne: {
            ...
            if (isReset) {
                // Remove any custom non-default items if resetting completely
                await MenuItem.deleteMany({});
                await MenuItem.insertMany(rawItems.map((item, idx) => ({ ...item, order: idx })));
            } else {
                await MenuItem.bulkWrite(bulkOps);
            }

            const freshItems = await MenuItem.find({}).sort({ order: 1, _id: 1 }).lean();

            return res.status(200).json({
                success: true,
                message: isReset ? 'Menu reset to factory defaults in MongoDB Atlas' : 'Full menu successfully synced to MongoDB Atlas',
        ...
        console.error('Error in /api/menu handler:', error);
